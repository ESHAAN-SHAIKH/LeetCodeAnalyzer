...
export const POST: APIRoute = async (context) => {
  ...
  // Mock cookie handlers for E2E tests only — never active in production, so a
  // real visitor whose session cookie happens to match these strings still gets
  // a genuine auth attempt instead of canned test data.
  const isTestMode = import.meta.env.MODE === 'test';

  if (isTestMode && cookie === 'mock_invalid_cookie') {
    return new Response(
      JSON.stringify({ error: 'auth_failed' }),
      { status: 401, headers: { 'Content-Type': 'application/json' } }
    );
  }
  if (isTestMode && cookie === 'mock_valid_cookie') {
    return new Response(
      JSON.stringify({
        topTopics: [
